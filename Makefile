...
IMAGE_TAG_BASE ?= quay.io/aknochow/ogo
...
.PHONY: generate-openshell-client
generate-openshell-client: buf protoc-gen-go protoc-gen-go-grpc ## Regenerate the internal/openshellclient gRPC client from proto/*.proto.
	cd proto && PATH="$(LOCALBIN):$$PATH" $(BUF) generate
...
image-build-multiarch: ## Build and push multi-arch container image.
	$(CONTAINER_TOOL) manifest create $(IMG)
	for platform in $$(echo $(PLATFORMS) | tr ',' ' '); do \
		$(CONTAINER_TOOL) build --platform $$platform -f Containerfile -t $(IMG)-$$platform . && \
		$(CONTAINER_TOOL) manifest add $(IMG) $(IMG)-$$platform; \
	done
