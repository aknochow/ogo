...
type OpenShellGatewaySpec struct {
	...
	// by CI when Dependabot bumps that pin.
	// +kubebuilder:default="0.0.92"
...
// +kubebuilder:validation:XValidation:rule="!(self.embedded && has(self.secretName) && size(self.secretName) > 0)",message="embedded and secretName are mutually exclusive"
